...
window.addEventListener("scroll", () => {
    ...
    let clientReview = document.querySelectorAll(".client-remark-container");
        clientReview = [...clientReview];
    const z = 100;

    for (const section of clientReview) {
        let sectionTop = section.getBoundingClientRect().top;
        let deviceHeight = window.innerHeight;

        if (sectionTop < deviceHeight - z) {
            section.classList.add("client-review-reveal");
        }
        else {
            section.classList.remove ("client-review-reveal");
        }
    }
})
